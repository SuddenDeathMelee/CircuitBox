...
pins
*/
void _74153::pin_to_output(logic_object* output, int pin_input, int pin_output){
...
*/
void _74153::input_to_pin(logic_object* input, int pin){
	inputs[pin]=input;
}
...
*/
bool _74153::evaluate_pin(int pin){
	/*
	The first and second elements of inputs are enable inputs. If a low signal is sent to one, their respective
	...
	*/
	...
	int sum=0;
	for(int i=2; i<4; i++){
		sum+= inputs[i]*pow(2,i-2);
	}
	if(pin)
		outputs[pin]=inputs[sum+3];
	else
		outputs[pin]=inputs[sum+8];
...
*/
bool _74153::restructure_pin(int pin){
...
*/
vector<bool> _74153::value(logic_object* obj){
...
*/
bool _74153::value(logic_object* obj, int pin_){
	int value;
	list<pin> pin_numbers= pins[obj];
	list<pin>::iterator pin_itr= pin_numbers.begin();
...
to a logic_object.
*/
bool _74153::output_connected(int pin){
