...
*/
bool _7402::evaluate_pin(int pin){
	outputs[pin]=true;
	
	vector<logic_object*> pins;
	/*
	To determine the output at pin, the pins 2*pin and 2*pin+1 of inputs. Note, the pin numbers
	do not correspond to the pin numbers of the actual chip here; the first two inputs of the _7402 corresponding to the first
	output of the chip.
	*/
	...
	for(int i=0; i<2; i++){
		if(pins[i]!=NULL){ //Make sure that a logic_object is actually connected to the chip
			if(pins[i]->is_chip()){
				...
				if(value){
					outputs[pin]=true;
					return outputs[pin]; //if a high input is found, then we need look no further
				}
			}
			else{
...
*/
bool _7402::restructure_pin(int pin){
...
*/
bool _7402::value(logic_object obj){
	int value;
	list<pin> pin_numbers= pins[obj];
	list<pin>::iterator pin_itr= pin_numbers.begin();
