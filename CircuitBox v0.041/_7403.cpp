...
pins
*/
void _7403::pin_to_output(logic_object* output, int pin_input, int pin_output){
...
*/
void _7403::input_to_pin(logic_object* input, int pin){
	inputs[pin]=input;
}
...
*/
bool _7403::evaluate_pin(int pin){
	outputs[pin]=false;
	
	vector<logic_object*> pins;
	/*
	To determine the output at pin, the pins 2*pin and 2*pin+1 of inputs. Note, the pin numbers
	do not correspond to the pin numbers of the actual chip here; the first two inputs of the _7403 corresponding to the first
	output of the chip.
	*/
...
*/
bool _7403::restructure_pin(int pin){
...
*/
vector<bool> _7403::value(logic_object* obj){
...
*/
bool _7403::value(logic_object* obj, int pin_){
	int value;
	list<pin> pin_numbers= pins[obj];
	list<pin>::iterator pin_itr= pin_numbers.begin();
...
to a logic_object.
*/
bool _7403::output_connected(int pin){
